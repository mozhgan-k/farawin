<template>
    <div>
        ...
        <div class="flex items-center justify-center bg-purple-200 p-3 border-t border-solid border-purple-300">
          <button class="rounded-md bg-purple-900 w-full text-white p-3 hover:opacity-75 focus:outline-none" @click="remove()">Delete</button>
        </div>
      </div>
    </div>
    </div>
...
export default defineComponent({
  ...
  methods: {
    remove () {
      const user = this.$props.deletuser
      removeUser(user).then(() => {
        this.$emit('removemem', false)
      })
    }
  }
})
